...
func NewFTPServer(listenPort, dataPortStart, dataPortEnd int, rootDir string) *FTPServer {
	...
	// Create enhanced user profiles with granular permissions and encrypted passwords
	...
	users := map[string]*UserProfile{
		...
		"anonymous": {
			PasswordHash: "", // Anonymous user has no password
			HomeDir:      "/public",
			Permissions: UserPermissions{
	...
	// 🔄 HYBRID APPROACH: Load users from file with fallback to defaults
	err := server.LoadUsersFromFile("users.json")
	if err != nil {
		// Create users.json with current defaults for future customization
		if saveErr := server.SaveUsersToFile("users.json"); saveErr != nil {
			log.Printf("Warning: Could not create default users.json: %v", saveErr)
		}
	}
...
func (session *ClientSession) handleCommand(command string) {
	...
	case strings.HasPrefix(siteCmdUpper, "MODE CHUNKED"):
		...
		case strings.HasPrefix(siteCmdUpper, "ADDUSER "):
		...
		case strings.HasPrefix(siteCmdUpper, "DELUSER "):
		...
		case strings.HasPrefix(siteCmdUpper, "LISTUSERS"):
			// SITE LISTUSERS command for listing all users
			session.handleListUsers()
			return

		case strings.HasPrefix(siteCmdUpper, "USERINFO "):
...
func (session *ClientSession) openMmapFile(filename string, writable bool) (*transfer.MmapFile, error) {
	// This function should not be needed since we're using SuperfastSTORHandler
	return nil, fmt.Errorf("openMmapFile not implemented")
}
...
// optimizedMmapUploadFixedSize - the core mmap upload method as specified in the analysis
func (session *ClientSession) optimizedMmapUploadFixedSize(dst *os.File, src io.Reader, size int64) (int64, error) {
	...
	if err != nil {
		return transfer.FallbackStreamingUpload(session, dst, src)
	}
